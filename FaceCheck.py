...
class FaceCheck(object):
    ...
    def face_search(self,_path):
        ...
        try:
            ...
            if not answer['error_code']:
                ...
                groun_id = answer['group_id']
                user_id = answer['user_id']
                score = answer['score']

                if int(score) >= 80 and user_id == 'pengkaibing':
                    print(groun_id, user_id, score)
                    self.next_deal()
    ...
    def TTS(self,text,spd=None,pit=None,vol=None,per=None,aue=None,tok=None):
        ...
        with open('PConfig/Welcome.mp3','wb') as f:f.write(res.content)
        os.startfile("PConfig\Welcome.mp3")
    ...
    def next_deal(self):
        self.play_media(r'E:\Programme\ACode\Python\API_Apply\FaceManage\PConfig\Welcome.mp3')

    def play_media(self,_file):
        player = QMediaPlayer()
        playlist = QMediaPlaylist()
        player.setPlaylist(playlist)
        if os.path.exists(_file):playlist.addMedia(QMediaContent(QUrl.fromLocalFile(_file)))
        else:playlist.addMedia(QMediaContent(QUrl(_file)))
        player.play()
        while True:
            play_time = player.position()
            time.sleep(0.1)
            if play_time == player.position() and play_time != 0:
                break
